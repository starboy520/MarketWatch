...
本文档按当前代码结构整理：**长期运行入口**为根目录 `main.py`，负责 X 时间线轮询、断点持久化，并将每条新推文交给 **LangGraph 编排层**（DeepSeek 分析 → 卡片渲染 → 飞书）。产品背景与分阶段规划见 [bloomberg_twitter_agent_design.md](./bloomberg_twitter_agent_design.md)；安装与安全配置见 [README.md](../README.md)、[配置与安全.md](./配置与安全.md)。
