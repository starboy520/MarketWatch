...
本文档按当前代码结构整理：**长期运行入口**为根目录 `main.py`，负责 **多数据源轮询**（当前：X 时间线；可选：**财联社电报** `ingestion/sources/cls_telegraph.py`）、断点持久化，并将每条标准化 **`TweetEvent`** 交给 **LangGraph 编排层**（DeepSeek 分析 → 卡片渲染 → 飞书）。产品背景与分阶段规划见 [bloomberg_twitter_agent_design.md](./bloomberg_twitter_agent_design.md)；安装与安全配置见 [README.md](../README.md)、[配置与安全.md](./配置与安全.md)。扩展新数据源时：在同一 checkpoint 文件内增加命名空间，产出与 `TweetEvent` 对齐的事件即可复用 pipeline。
...
| `ingestion/checkpoints.py` | `PollCheckpointStore`；嵌套键 `x`（账号→since_id）、`cls_telegraph`（游标与去重） |
...
| `docs/配置与安全.md` | 密钥、环境变量、X Bearer（代码内无默认 Token） |
| `docs/README.md` | 文档索引 |
...
2. **`PollCheckpointStore`** 读写 `poll.checkpoint_file`（默认 `checkpoints.json`）。新格式含 **`x`**（各 X 账号 `since_id`）与可选 **`cls_telegraph`**（财联社 refresh 游标、`seen_ids`）；旧文件仅账号名为顶层键时会自动迁移到 `x` 下。
...
- **设计对照**：`ingestion/models.py`、`pipeline/state.py` 文档字符串与 `docs/bloomberg_twitter_agent_design.md` 中的事件与状态约定。
