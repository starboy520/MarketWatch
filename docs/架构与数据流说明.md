...
- **`market_retriever`**：仅在 `nodes.py` 占位，**未** `add_node` 进图；预留与 `market_map` / `market_impact` 对接。

条件路由 **`_EdgeRouter.after_relevance`**：仅当节点返回 **`status == "filtered"`** 时走向 `END`，与节点内放行规则一致。

**图走向（Mermaid 与条件边表）**见 [LangGraph编排说明.md](./LangGraph编排说明.md) **§4.0**（与 `pipeline/graph.py` 一致）。
