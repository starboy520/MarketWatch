...
- **输入**：`raw_text`。
- **逻辑**：
  - 仅当 **`is_relevant`** 且 **`confidence`** ≥可配置阈值时放行（见 `PipelineCompileConfig.relevance_confidence_threshold`）；
...
### 4.0 本仓库当前实现的路径与条件边（`pipeline/graph.py`）

以下与 **`TweetPipelineCompiler._wire_entry_and_relevance` / `_wire_publish_and_retry`** 及 **`_EdgeRouter`** 一致；便于对照源码排查「为何没发飞书」「为何重试」。

#### 默认线性顺序（`enable_prefilter=False`）

```text
START → relevance_filter → body_translate → card_renderer → feishu_publisher
```

随后由 **`publish_status`** 决定是否结束或进入退避重试（见下表）。**`market_retriever`**：`nodes.py` 仅有占位实现，**未**在 `StateGraph.add_node` 中注册，边中未插入；演进时插在 `relevance_filter` 与 `body_translate` 之间即可。

#### 流程图（当前默认：无 Prefilter、无 Market_Retriever）

```mermaid
flowchart TD
  START([START]) --> RF[relevance_filter]
  RF -->|status == filtered| END1([END])
  RF -->|否则| BT[body_translate]
  BT --> CR[card_renderer]
  CR --> FP[feishu_publisher]
  FP -->|publish_status ok| END2([END])
  FP -->|publish_status retry| RB[retry_backoff]
  FP -->|publish_status failed| END3([END])
  RB -->|status == dead_letter| END4([END])
  RB -->|否则| FP
```
...
| 位置 | 函数 | 规则 |
|------|------|------|
| `relevance_filter` 出口 | `_EdgeRouter.after_relevance` | 仅当 `state["status"] == "filtered"` → **END**；否则 → **`body_translate`**（与节点内是否写 `llm_done` 等无关，只要不是 `filtered` 即继续）。 |
| `relevance_filter` 节点内 | `make_relevance_filter` | 放行需 **`is_relevant` 且 `confidence` ≥ `relevance_confidence_threshold`（默认 0.7）**；否则返回 `status: filtered`。 |
| `feishu_publisher` 出口 | `_EdgeRouter.after_feishu` | `publish_status == "ok"` → **END**；`"retry"` → **`retry_backoff`**；否则 → **END**（不重试）。 |
| `retry_backoff` 出口 | `_EdgeRouter.after_retry_backoff` | `status == "dead_letter"`（超过 `max_feishu_retries`）→ **END**；否则 → 回到 **`feishu_publisher`**。 |

#### 可选：`enable_prefilter=True`

```text
START → prefilter →（filtered → END | continue → relevance_filter）→ 同上
```

`after_prefilter` 与 `after_relevance` 相同：**仅** `status == "filtered"` 时结束。
...
**本仓库当前默认（与上图差异）**：`pipeline/graph.py` 中 **未** 在边里插入 `Market_Retriever`；实际边为 `relevance_filter → body_translate → card_renderer → feishu_publisher`（`Market_Retriever` 为预留，见代码注释）。**精确走向与条件边**见上文 **§4.0**。上图表示目标拓扑，接入龙头映射后可在 `relevance_filter` 与 `body_translate` 之间插入该节点。

### 4.2 目标拓扑下的补充条件（与实现对照）

| 规则 | 行为 |
|------|------|
| `Relevance_Filter` 在节点内未放行（`is_relevant` 为 false 或 `confidence` 低于阈值）→ `status=filtered` | 条件边直达 **END**，不跑后续节点 |
