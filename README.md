...
- 配置：`cp config.example.toml config.toml`，按 **[docs/配置与安全.md](docs/配置与安全.md)** 填写密钥与 **X Bearer**（必填其一：`[x] public_bearer_token` 或 `NEWS_AGENT_X_BEARER`）；财联社见 `config.example.toml` 中 `[sources.cls_telegraph]`
- 运行：`python main.py`
