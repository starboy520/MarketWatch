# 编排层详细设计：LangGraph 状态机

> **范围**：只描述 [bloomberg_twitter_agent_design.md](./bloomberg_twitter_agent_design.md) 中的 **Processing Pipeline（处理编排）** 用 LangGraph 落地时的状态、节点、边与恢复策略。  
...
- **`analysis` 与主设计 §5**：落地时可直接采用 `relevance` + `sentiment` + `market_impact` 三节 JSON；为贴近你最初一版文档，表中保留 **`analysis`** 作为「LLM 一坨输出」的兼容名，实现时再映射到 §5 字段名。
- **`graph thread_id`**：建议 `f"tweet:{tweet_id}"`，与 Checkpointer 组合保证「同帖重放」不重复开逻辑线程。
...
- **职责**：**主路径 LLM**——是否与中国宏观、AI、半导体、光模块等相关；输出 **结构化 JSON**（`Structured Output` / JSON Schema），含 **置信度**。
- **输入**：`raw_text`。
- **逻辑**：
...
| 主题 | 约定 |
|------|------|
| **Checkpointer** | LangGraph `SqliteSaver` / `PostgresSaver`；推送失败时从「渲染完成之后」恢复，减少重复耗 Token（与你原稿一致） |
...
若冲突，**以 [bloomberg_twitter_agent_design.md](./bloomberg_twitter_agent_design.md) 为准**。
...
*下一步实现时：在仓库新增例如 `pipeline/graph.py` 构建 `StateGraph`，由 `main` 或异步 worker 在 `TweetEvent` 上 `invoke`；本文不展开具体 LangGraph API 版本与代码骨架。*
